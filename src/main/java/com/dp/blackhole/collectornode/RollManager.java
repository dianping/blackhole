...
public class RollManager {
    ...
    public void init(String hdfsbase, String suffix, int port) throws IOException {
    ...
    public boolean doRegister(String app, String source, long period, RollPartition roll) {
        ...
        if (rolls.get(ident) == null) {
            ...
            Broker.getSupervisor().sendMsg(message);
            ret = true;
        } else {
    ...
    public boolean doUpload(RollID rollID) {
        ...
        // TODO, submit a launch work to poll, and clear entry in roll if success
        HDFSUpload_new upload = new HDFSUpload_new(this, fs, ident, roll);
        pool.execute(upload);
        return true;
    }
    
    private RollIdent getRollIdent(String app, String source, long period) {
        Date time = new Date(Util.getClosestRollTs(Util.getTS(), period));
